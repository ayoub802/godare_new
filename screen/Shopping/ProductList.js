...
const ProductList = () => {
    ...
    const handleCartLogin = async (product) => {
      ...
      if (keys.length != numberAttributes)
      {
        ...
        ToastAndroid.show("Vous devez selectionner tous les attributs du produit",ToastAndroid.SHORT)
        return;
      }
    ...
    const findMatchingStock = (product, attributes) => {
      ...
      if (Object.keys(attributes).length == product.attributs.length)
      {
        ...
        if (!matchFound) {
          ...
          ToastAndroid.show('Rupture de stock',ToastAndroid.SHORT)
        }
      }
    };
    ...
    const RenderPrices = props => {
      ...
      return (
        t('A partir de') + ' ' + minPrice + '€' + ( product.unite ? (' / ' + product.unite.valeur) : '' )
      );
    }
