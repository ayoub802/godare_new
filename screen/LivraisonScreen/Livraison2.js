...
import { getAuthUserEmail, getLivraisonValues, getMagasins, getNewAddedAddress, getPlatformLanguage, getPanier, getRemiseUsed, getSelectedCountry, getSelectedService, getServices, saveLivraisonAdresseId, saveLivraisonDomicileData, saveLivraisonMagasinData, saveLivraisonMode, saveLivraisonPrices, saveLivraisonRelaisId, saveSelectedService, saveMagasins, saveSelectedCountry } from '../../modules/GestionStorage';
...
function DeliveryMethodsU(props) {
    ...
    async function fetchData()
    {
      ...
      try 
      {
        ...
        formattedUserAdresses = response.data.map(ls=>{
          return {id:ls.id, label:(ls.adresse + ' ' + ls.codePostal + ' ' + ls.ville + ' ' + ls.pays), value:ls.id,}
        });
  ...
  const NavigateToUserAddress = () => {
    props.navigation.navigate(Navigationstrings.AddAddress, {pageFrom: 'livraison', email: UserEmail, paysDepartLivraison: paysLivraisonObject.destination});
  }
  ...
  async function NavigateToMagasin(){
    ...
    if (NomContact === '')
    {
      Toast.show({
        type: 'error',
        text1: 'Nom contact',
        text2: t('Le nom de la personne qui récupère la commande est obligatoire'),
      });
    ...
    if (TelContact === '')
    {
      Toast.show({
        type: 'error',
        text1: 'Téléphone contact',
        text2: t('Le téléphone de la personne qui récupère la commande est obligatoire'),
      });
